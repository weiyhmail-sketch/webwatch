...
## 🔴 已切 LIVE 实盘（2026-06-12）

- 用户创建 secrets.live.env（4001/U 账户）、Gateway live 登录、API 4001 + 非只读 + 跳过委托单预防。
- 二重互锁启动验证通过：`WEBWATCH_ENV=live WEBWATCH_LIVE_CONFIRM=YES`，is_live=True，净值 ~$25.5k。
- **AAPL 实时行情正常**（Gateway 即 live 会话，10197 不再出现）。雷达扫描 162（live 扫描权限）不影响下单。
- **金额上限 + 最大亏损上限已取消**（用户在明示 PDT $25k 风险后两次确认；live 当天）。
  保留：股数上限 10 万、账户失效 $1k 保护、PDT 提示。金额天花板=IBKR 保证金。
- 插曲：① Gateway 配置保存时「只读API」被误勾 → IB 321 只读拒单，用户解除后恢复；
  ② 账户存在面板外的原有持仓 XFAB 4,560 股（欧股 Euronext，EUR）——面板只做美股(SMART/USD)，
  无法订阅/保护/平仓该仓位，用户明确选择继续持有，由其自行渠道管理。
- PDT 警示已多次传达：净值 $25.5k 距 $25k 仅咫尺，单笔大亏即触发日内交易限制。
